...
from Apollo.Ensemble_Missioner.Stacking_Missioner import Stacking_Missioner


# -------------------- Stacking classifier model --------------------
class StackingClassifier_Model(Stacking_Missioner):
    """
    Train, evaluate, and visualize a stacking-based classification model.

    This class provides the model-layer workflow for classification tasks based on
    ``StackingClassifier`` through the inherited stacking mission pipeline. It is
    responsible for declaring the task type, defining classification-specific
    scoring behavior, training the stacking ensemble, evaluating fitted models,
    ...
    - mission layer
        Handles stacking-specific estimator construction, validation, prediction
    ...
    Notes
    -----
    This class delegates stacking model construction and fitting to the inherited
    ...
    def task(self) -> str:
        ...
        probability-based stacking methods, and evaluation rules should be used.
        """
    ...
    def multioutput_scorer(y_true, y_pred) -> float:
        ...
        y_true : Any
            ...
            - ``pandas.Series`` for single-output classification
            - one-dimensional ``numpy.ndarray`` for single-output classification
            - ``pandas.DataFrame`` for multi-output classification
            - array-like structures convertible to ``pandas.DataFrame``

        y_pred : Any
            Predicted target values produced by the model.

            For multi-output classification, this input is aligned to the target
            column structure of ``y_true`` when needed.
        ...
        For multi-output classification, the method computes the weighted F1 score
    ...
    def train(
        ...
        """
        Train a stacking classifier and return a full training summary.

        This method fits a classification stacking ensemble using the inherited
        ``stacking_fit_engine()`` workflow, then evaluates the fitted model on
        both training and test sets through ``model_evaluation_engine()``.

        Parameters
        ----------
        estimators : Sequence[Tuple[str, Any]]
            First-layer estimator collection.

            Each item must be a ``(name, estimator)`` pair. Estimators should be
            scikit-learn compatible and appropriate for classification tasks.
        ...
        stacking_cv : int, default=5
            Number of internal cross-validation folds used by the stacking
            estimator to generate out-of-fold meta-features for the final
            estimator.
        ...
        stack_method : str, default="auto"
            Method used by ``StackingClassifier`` to generate first-layer outputs
            for meta-learning.

            Typical options depend on estimator capabilities and may include
            methods such as prediction labels, probabilities, or decision scores.

        param_grid : dict[str, Any] or None, default=None
            Parameter grid passed to GridSearchCV for outer pipeline search.

            Grid keys should follow the naming convention defined by the parent
            pipeline structure.

        random_state : int, default=42
            Random seed used for inherited outer CV splitter construction where
            applicable.

        extra_steps : list[tuple[str, Any]] or None, default=None
            Optional pipeline steps inserted between preprocessing and the final
            stacking classifier in the outer pipeline.

        use_preprocess : bool, default=True
            Whether to build and use the outer preprocessing pipeline supplied by
        ...
        dict[str, Any]
            ...
            - fit summary returned by ``stacking_fit_engine()``
        ...
        fit_summary = self.stacking_fit_engine(
    ...
    def model_evaluation_engine(self) -> Dict[str, Any]:
        """
        Evaluate the fitted stacking classifier on training and test datasets.
...
class StackingRegressor_Model(Stacking_Missioner):
    """
    Train and evaluate a stacking-based regression model.

    This class provides the model-layer workflow for regression tasks based on
    ``StackingRegressor`` through the inherited stacking mission pipeline. It is
    responsible for declaring the task type, defining regression-specific scoring
    behavior, training the stacking ensemble, and evaluating fitted models.

    The class supports both single-output and multi-output regression tasks. For
    multi-output regression, evaluation metrics are computed separately for each
    ...
    def multioutput_scorer(y_true, y_pred) -> float:
        ...
        For multi-output regression, it computes the R² score separately for each
        target column and returns the arithmetic mean across all target columns.
        ...
        y_true : Any
            ...
            - ``pandas.Series`` for single-output regression
            - one-dimensional ``numpy.ndarray`` for single-output regression
            - ``pandas.DataFrame`` for multi-output regression
            - array-like structures convertible to ``pandas.DataFrame``

        y_pred : Any
            Predicted target values produced by the model.

            For multi-output regression, this input is aligned to the target
            column structure of ``y_true`` when needed.
        ...
        Notes
        -----
        For single-output regression, this method computes:

        - ``r2_score(y_true, y_pred)``

        For multi-output regression, the method computes R² separately for each
        target column and then returns the arithmetic mean across all targets.
    ...
    def train(
        ...
        """
        Train a stacking regressor and return a full training summary.

        This method fits a regression stacking ensemble using the inherited
        ``stacking_fit_engine()`` workflow, then evaluates the fitted model on
        both training and test sets through ``model_evaluation_engine()``.
        ...
        estimators : Sequence[Tuple[str, Any]]
            ...
            Each item must be a ``(name, estimator)`` pair. Estimators should be
            scikit-learn compatible and appropriate for regression tasks.
        ...
        extra_steps : list[tuple[str, Any]] or None, default=None
            Optional pipeline steps inserted between preprocessing and the final
            stacking regressor in the outer pipeline.
        ...
        Internally, ``stack_method="auto"`` may still be forwarded for interface
        consistency, but stack-method behavior is classification-specific and is
        ignored in regression stacking workflows.
        """
        if self.X_train is None or self.Y_train is None:
            raise ValueError("⚠️ Run train_test_split_engine() before training ‼️")
        ...
        fit_summary = self.stacking_fit_engine(
            ...
            n_jobs=n_jobs,
            passthrough=passthrough,
            stack_method="auto",  # ignored automatically inside StackingRegressor flow
            param_grid=param_grid,
            random_state=random_state,
            extra_steps=extra_steps,
            use_preprocess=use_preprocess,
    ...
    def model_evaluation_engine(self) -> Dict[str, Any]:
        """
        Evaluate the fitted stacking regressor on training and test datasets.
