...
from Apollo.Ensemble_Missioner.AdaBoosting_Missioner import AdaBoost_Missioner


# -------------------- AdaBoost classifier model --------------------
class AdaBoostClassifier_Model(AdaBoost_Missioner):
    """
    Train, evaluate, and visualize an AdaBoost-based classification model.

    This class provides the model-layer workflow for classification tasks based on
    ``AdaBoostClassifier`` through the inherited AdaBoost mission pipeline. It is
    responsible for declaring the task type, defining classification-specific
    scoring behavior, training the AdaBoost ensemble, evaluating fitted models,
    ...
    - mission layer
        Handles AdaBoost-specific estimator construction, shared training flow,
    ...
    Notes
    -----
    This class delegates AdaBoost model construction and fitting to the inherited
    ...
    def train(
        ...
        """
        Train an AdaBoost classifier and return a full training summary.

        This method fits a classification AdaBoost model using the inherited
        ``adaboost_fit_engine()`` workflow, then evaluates the fitted model on
        ...
        estimator : Any
            ...
            This estimator must be scikit-learn compatible and appropriate for
        ...
        extra_steps : list[tuple[str, Any]] or None, default=None
            ...
            AdaBoost classifier in the outer pipeline.
        ...
        dict[str, Any]
            ...
            - fit summary returned by ``adaboost_fit_engine()``
        ...
        fit_summary = self.adaboost_fit_engine(
            estimator=estimator,
...
class AdaBoostRegressor_Model(AdaBoost_Missioner):
    """
    Train and evaluate an AdaBoost-based regression model.

    This class provides the model-layer workflow for regression tasks based on
    ``AdaBoostRegressor`` through the inherited AdaBoost mission pipeline. It is
    responsible for declaring the task type, defining regression-specific
    scoring behavior, training the AdaBoost model, and evaluating fitted models.
    ...
    def train(
        ...
        """
        Train an AdaBoost regressor and return a full training summary.

        This method fits a regression AdaBoost model using the inherited
        ``adaboost_fit_engine()`` workflow, then evaluates the fitted model on
        both training and test sets through ``model_evaluation_engine()``.
        ...
        extra_steps : list[tuple[str, Any]] or None, default=None
            Optional pipeline steps inserted between preprocessing and the final
            AdaBoost regressor in the outer pipeline.
    ...
    def model_evaluation_engine(self) -> Dict[str, Any]:
        """
        Evaluate the fitted AdaBoost regressor on training and test datasets.
